...
internal static class ShipLogPacking
{
    public static void Apply(IModBehaviour[] jamEntries)
    {
        ...
        foreach (var jamEntry in jamEntries)
        {
            try
            {
                var starSystem = jamEntry.ModHelper.Storage.Load<StarSystemConfig>(Path.Combine("systems", $"{ModJam5.SystemName}.json"));
        ...
        var finalEntryPositions = Main.SystemDict[ModJam5.SystemName].Config.entryPositions;
